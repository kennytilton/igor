#!/usr/bin/env python3
"""
textcard.py - Render a styled text card to MP4.

Usage:
    python3 textcard.py "Your text here." [options]
    python3 textcard.py --file card.txt [options]

When --file is used, each line in the file becomes exactly one rendered line
(no re-wrapping). Blank lines are honored as vertical spacers.

Options:
    --file PATH      Text file to render (overrides positional text arg)
    --out FILE       Output MP4 filename (default: textcard.mp4)
...
def render_frame(lines, width, height, fontsize, bg, fg, font_path):
    ...
    # Total block height — blank lines count as half
...
def main():
    ...
    ap.add_argument("text", nargs="?", default=None,
                    help="Text string (overridden by --file)")
    ap.add_argument("--file",     default=None,
                    help="Text file; each line rendered as-is, blank lines = spacer")
    ap.add_argument("--out",      default="textcard.mp4")
    ...
    if args.file:
        if not os.path.isfile(args.file):
            print(f"textcard error: file not found: {args.file!r}", file=sys.stderr)
            sys.exit(1)
        ...
        from PIL import Image, ImageDraw, ImageFont
        tmp_img = Image.new("RGB", (args.width, args.height))
        tmp_draw = ImageDraw.Draw(tmp_img)
        try:
            font = ImageFont.truetype(font_path, args.fontsize) if font_path else ImageFont.load_default()
        except Exception:
            font = ImageFont.load_default()
        max_w = int(args.width * 0.70)
        lines = wrap_text(tmp_draw, args.text, font, max_w)
